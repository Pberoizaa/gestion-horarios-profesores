                                  ...
                                  <div className="item-content">
                                    {isBloqueado ? (
                                      <>
                                        ...
                                        {item.curso && <span className="course" style={{ opacity: 0.7, fontSize: '0.7rem' }}>{item.curso}</span>}
                                      </>
                                    ) : (
                                      <>
                                        {item.isInherited && (
        ...
        <div className="modal-overlay">
          <div className="modal-content schedule-modal">
            ...
            <form onSubmit={handleSaveBlock}>
              ...
              {['clase', 'tc', 'administrativo', 'dupla', 'apoderado', 'bloqueado'].includes(newBlock.tipo_bloque) && (
                <div className="form-group">
                  <label>
                    {newBlock.tipo_bloque === 'clase' ? 'Curso' : 
                     newBlock.tipo_bloque === 'bloqueado' ? 'Motivo (opcional)' :
                     'Información Adicional (Con quién / Detalle)'}
                  </label>
                  <input 
                    type="text" 
                    value={newBlock.curso || ''} 
                    placeholder={newBlock.tipo_bloque === 'clase' ? "Ej: 1°A, 4°B..." : 
                                 newBlock.tipo_bloque === 'bloqueado' ? "Ej: No trabaja los viernes" :
                                 "Ej: Gladys A., Reunión UTP..."}
          ...
          <div className="modal-content" style={{ maxWidth: '800px', width: '90%' }}>
            ...
            <div style={{ marginBottom: '1.5rem', display: 'flex', gap: '1rem', alignItems: 'center' }}>
               <div className="stat-card" style={{ flex: 1, padding: '1rem', textAlign: 'center', background: 'var(--bg-soft)', borderRadius: '1rem' }}>
                  <span style={{ fontSize: '0.8rem', opacity: 0.7, fontWeight: 600 }}>COBERTURAS PUNTUALES</span>
                  <p style={{ fontSize: '2rem', fontWeight: 800, margin: 0, color: 'var(--accent)' }}>
                    {summaryCoverages.filter(c => c.tipo === 'cobertura').length}
                  </p>
               </div>
               <button 
                  className="btn-edit" 
